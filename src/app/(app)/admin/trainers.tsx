...
export default function TrainersScreen() {
  ...
  const trainers = useMemo(
    () =>
      profiles
        .filter((p) => p.status === 'approved' && p.role === 'trainer')
        .filter((p) => {
    ...
    <AdminScrollScreen includeTopInset={false}>
      <View style={styles.headerRow}>
        <View style={styles.headerLeft}>
          <Text style={styles.eyebrow}>ENTRENADORES</Text>
          <Text style={styles.title}>{trainers.length} entrenadores</Text>
        </View>
        {canCreateMember ? (
          <Pressable
            onPress={() => router.push('/create-member?role=trainer')}
          ...
          {trainers.map((profile) => (
            <Pressable
              ...
              <AdminCard style={styles.item}>
                <InitialAvatar firstName={profile.first_name} lastName={profile.last_name} staff />
